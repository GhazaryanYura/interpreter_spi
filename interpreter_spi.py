...
class Lexer(object):
    def __init__(self, text):
        # client string input e.g. "4 * 6", "15 / 5 * 2", etc
        self.text = text;
...
class Parser(object):
    ...
    def procedure_declaration(self):
        """procedure_declaration :
            PROCEDURE ID (LPAREN format_parameter_list RPAREN)? SEMI block SEMI
        """
    ...
    def proccall_statement(self):
        """proccall_statment : ID LPAREN (expr (COMMA expr)*)? RPAREN"""
        token = self.current_token;
        proc_name = self.current_token.value;
        self.eat(TokenType.ID);
        self.eat(TokenType.LPAREN);
    ...
    def parse(self):
        ...
        variable_declaration : ID (COMMA ID)* COLON type_spac
...
# SYMBOLS, TABLE, SEMANTIC ANALYSIS
...
class ProcedureSymbol(Symbol):
    ...
    def __str__(self):
        ...
        return "<{self.__class__.__name__}(name={self.name}, parameters={self.formal_params})>"
        __repr__ = __str__;
...
class SemanticAnalyzer(NodeVisitor):
    ...
    def visit_VarDecl(self, node):
        ...
        # We have all the information we need to create variable symbol.
...
class CallStack:
    ...
    def push(self, ar): # ar == ActivationRecords
